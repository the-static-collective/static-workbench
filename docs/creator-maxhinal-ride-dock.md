...
4. HOUSE checks the bounded JSON shape (format, corpus reference, seed, Slice gas, operation/output identity and non-promotion fields). It displays the **self-reported** corpus and replay values, modes, operation references, residuals and bad-spin reasons, and hashes the *exact pasted UTF-8 bytes*.
